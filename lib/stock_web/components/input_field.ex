defmodule StockWeb.Components.InputField do
  @moduledoc """
  The `StockWeb.Components.InputField` module provides a customizable input field component that
  ...
  def input_field(%{type: "checkbox"} = assigns) do
    assigns =
  ...
  def input_field(%{type: "select"} = assigns) do
    ~H"""
    <div>
  ...
  def input_field(%{type: "textarea"} = assigns) do
    ~H"""
    <div>
      <.label for={@id}><%= @label %></.label>
      <textarea
  ...
  def error(assigns) do
    ...
    <p class="mt-3 flex gap-3 text-sm leading-6 text-rose-600">
      <.icon name="fa-exclamation-circle-mini" class="mt-0.5 h-5 w-5 flex-none" /> <%= render_slot(
        @inner_block
      ) %>
    </p>
    """
