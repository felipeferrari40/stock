defmodule StockWeb.Components.Alert do
  @moduledoc """
  StockWeb.Components.Alert module provides collection of alert components and helper functions for managing and displaying alerts
  ...
  use Gettext, backend: StockWeb.Gettext
  ...
  defp color_variant("default", type) when type in [:info, :success] do
  ...
  defp color_variant("outline", :white) do
    "text-white border border-white hover:[&>button]:text-[#787878]"
  end

  defp color_variant("outline", :primary) do
    "text-[#4363EC] border border-[#4363EC] hover:[&>button]:text-[#072ed3] "
  end

  defp color_variant("outline", :secondary) do
    "text-[#6B6E7C] border border-[#6B6E7C] hover:[&>button]:text-[#60636f]"
  end

  defp color_variant("outline", type) when type in [:info, :success] do
    "text-[#227A52] border border-[#6EE7B7] hover:[&>button]:text-[#50AF7A]"
  end

  defp color_variant("outline", :warning) do
    "text-[#FF8B08] border border-[#FF8B08] hover:[&>button]:text-[#FFB045]"
  end

  defp color_variant("outline", type) when type in [:error, :danger] do
    "text-[#E73B3B] border border-[#E73B3B] hover:[&>button]:text-[#F0756A]"
  end

  defp color_variant("outline", :info) do
    "text-[#004FC4] border border-[#004FC4] hover:[&>button]:text-[#3680DB]"
  end

  defp color_variant("outline", :misc) do
    "text-[#52059C] border border-[#52059C] hover:[&>button]:text-[#8535C3]"
  end

  defp color_variant("outline", :dawn) do
    "text-[#4D4137] border border-[#4D4137] hover:[&>button]:text-[#FFECDA]"
  end

  defp color_variant("outline", :light) do
    "text-[#707483] border border-[#707483] hover:[&>button]:text-[#A0A5B4]"
  end

  defp color_variant("outline", :dark) do
    "text-[#1E1E1E] border border-[#1E1E1E] hover:[&>button]:text-[#787878]"
  end
  ...
  defp color_variant("unbordered", type) when type in [:info, :success] do
  ...
  defp color_variant("shadow", type) when type in [:info, :success] do
